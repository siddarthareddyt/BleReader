package com.mcc.ul.example.dbitin;
...
import com.mcc.ul.example.dbitin.NetDiscoveryInfoDialog;
import com.mcc.ul.example.dbitin.NetDiscoveryInfoDialog.NoticeDialogListener;
import com.mcc.ul.example.dbitin.R;
...
/*
 * Library Call Demonstrated:    DioDevice.dBitIn()
   ...
   Demonstration:                Configures the selected bit or the selected port 
                                 ...
                                 reads and displays the bit value.
   ...
   7. Configure the bit as input ((if necessary)
   8. Call DaqDevice.dBitIn() to read the port
...
public class DBitInActivity extends Activity {
	...
	RadioButton mBitStateRadioButton;
    ...
    private void configureSelectedBit() {
    	PortInfo portInfo = (PortInfo) mPortSpinner.getSelectedItem();
    	int bitNum = (Integer) mBitSpinner.getSelectedItem();
    	...
    	if(portInfo.getPortIOType() == DigitalPortIOType.BITIO || portInfo.getPortIOType() == DigitalPortIOType.IO) {
    		...
    		try {
    			...
    			if(portInfo.getPortIOType() == DigitalPortIOType.BITIO)
    				mDioDevice.dConfigBit(portInfo.getType(), bitNum, DigitalDirection.INPUT);
    ...
    private void bitInTimer(){
    	
    	PortInfo portInfo = (PortInfo) mPortSpinner.getSelectedItem();
    	final int bitNum = (Integer) mBitSpinner.getSelectedItem();
    	
    	try {
    		synchronized(this) {
    			if(mDioDevice != null) {
	    		// Read the specified bit
	    		final boolean bitState = mDioDevice.dBitIn(portInfo.getType(), bitNum );
	    		
	    		runOnUiThread(new Runnable() {
					@SuppressLint("DefaultLocale")
					public void run() {
						updateStatus("Reading bit " + bitNum + ". Input a TTL logic level at the specified bit to change the bit Value" , false);
						
						mBitStateRadioButton.setChecked(bitState);
						
						if(bitState)
							mBitStateRadioButton.setText("High");
						else
							mBitStateRadioButton.setText("Low");
					}
				});
    			}
    		}
    ...
    private void startBitInTimer() {
    	...
    	mBitInTimer = new Timer();
		mBitInTimer.schedule(new TimerTask() {          
	        @Override
	        public void run() {
	        	bitInTimer();
	        }
    ...
    private void stopBitInTimer() {
    	if(mBitInTimer != null) {
    		mBitInTimer.cancel();
    		
    		updateStatus("Read operation stopped", false);	
    	}
