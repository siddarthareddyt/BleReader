package com.mcc.ul.example.ainscan.exttrigger;
...
import com.mcc.ul.example.ainscan.exttrigger.NetDiscoveryInfoDialog;
import com.mcc.ul.example.ainscan.exttrigger.NetDiscoveryInfoDialog.NoticeDialogListener;
import com.mcc.ul.example.ainscan.exttrigger.R;
...
/*
 * Library Call Demonstrated:    AiDevice.setTrigger()

   Purpose:                      Selects the Trigger source. This trigger is
                                 used to initiate A/D conversion using
                                 AiDevice.aInScan(), with
                                 AiScanOption.EXTTRIGGER Option.

   Demonstration:                Selects the trigger source
                                 Displays the analog input on selected channels.
                                 
   Other Library Calls:          AiDevice.getStatus()
                                 AiDevice.StopBackground()

   Special Requirements:         Selected device must have digital triggering source and type.
                                 Selected device must have an A/D converter.
   ...
   7. Call AiDevice.setTigger to set the trigger type
   ...
   10. Call AiDevice.stopBackground() to stop the scan
...
public class AInScanExtTriggerActivity extends Activity {
    ...
    public class OnDaqDeviceSelectedListener implements OnItemSelectedListener {    
    	public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {      
        	...
        	if(devInfo.hasAiDev()) {
        		...
        		// Get Supported digital triggers	
        		EnumSet<TriggerType> digitalTrigTypes = aiInfo.getTriggerTypes(TriggerSourceType.DIGITAL);
        		
        		if(digitalTrigTypes.size() > 0)
        			mTrigTypeAdapter.addAll(digitalTrigTypes);
        		else
                	updateStatus("Selected device does not support digital trigger source", true);
    ...
    void startAInScan()
    {
    	...
    	double rate = Double.parseDouble(mRateEditText.getText().toString());
    	EnumSet<AiScanOption> options = EnumSet.of(AiScanOption.EXTTRIGGER); // select EXTTRIGGER option
    	...
    	// setTrigger arguments
    	TriggerType triggerType = (TriggerType) mTrigTypeSpinner.getSelectedItem();
    	
    	// the following arguments are ignored when a digital trigger type specified
    	int triggerChannel = 0; 
    	double lowThreshold = 0, highThreshold = 0;
    	...
    	try {
			...
			//  Select the trigger source using DaqDevice.setTrigger()
    		mAiDevice.setTrigger(triggerType, triggerChannel, lowThreshold, highThreshold, null, retriggerCount);	
			...
			public void run() {
				...
				if(scanStatus.currentStatus == Status.IDLE) {
					...
					if(scanStatus.currentIndex >= 0)
						...
						updateStatus("Scan is running. Waiting for external trigger", false);
				}
			}
		});
    ...
    private void updateActivity() {
    	...
    	if(!mDaqDevInventoryAdapter.isEmpty()) {
        	...
        	mTrigTypeSpinner.setEnabled(true);
