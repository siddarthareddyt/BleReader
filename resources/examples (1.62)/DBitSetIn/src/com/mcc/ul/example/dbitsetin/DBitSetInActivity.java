package com.mcc.ul.example.dbitsetin;
...
import com.mcc.ul.example.dbitsetin.NetDiscoveryInfoDialog;
import com.mcc.ul.example.dbitsetin.NetDiscoveryInfoDialog.NoticeDialogListener;
import com.mcc.ul.example.dbitsetin.R;
...
/*
 * Library Call Demonstrated:    DioDevice.dBitSetIn()
   ...
   8. Call DioDevice.dBitSetIn() to read the port
...
public class DBitSetInActivity extends Activity {
	...
	RadioButton[] mBitValRadioButton;
    ...
    private void dInTimer(){
    	...
    	final PortInfo portInfo = (PortInfo) mPortSpinner.getSelectedItem();
    	
    	updateStatus("Reading " + portInfo + ". Input a TTL logic level at the bits to change the bits Value" , false);
    	...
    	try {
    		...
    		synchronized(this) {
    			if(mDioDevice != null) {
		    		...
		    		final BitSet bitset = mDioDevice.dBitSetIn(portInfo.getType());
		    		
		    		runOnUiThread(new Runnable() {
						@SuppressLint("DefaultLocale")
						public void run() {
							
							for(int i = 0; i < portInfo.getNumBits(); i++)
								mBitValRadioButton[i].setChecked(bitset.get(i));
						}
					});
    			}
    		}
    ...
    private void initActivity() {
    	...
    	mBitsLayout = (LinearLayout) findViewById(R.id.layout_bits);
    ...
    private void startDInTimer() {
    	...
    	PortInfo portInfo = (PortInfo) mPortSpinner.getSelectedItem();
    	addRadioButtons(portInfo.getNumBits());
    ...
    void addRadioButtons(int bitCount) {
    	
    	mBitsLayout.removeAllViews();
    	
    	mBitTextView = new TextView[bitCount];
    	mBitValRadioButton = new RadioButton[bitCount];
		
		LinearLayout[] horizontalLayout = new LinearLayout[bitCount];
		
		for(int i = 0; i < bitCount; i++) {
			horizontalLayout[i]= new LinearLayout(this);
			mBitTextView[i] = new TextView(this);
    		mBitTextView[i].setTextColor(Color.parseColor("#0E139F"));
    		mBitTextView[i].setText("bit " + i + ":");
    		
    		mBitValRadioButton[i] = new RadioButton(this);
    		mBitValRadioButton[i].setHeight(mStatusTextView.getHeight());
    		mBitValRadioButton[i].setEnabled(false);
    		
    		mBitTextView[i].setGravity(Gravity.RIGHT);
    		...
    		horizontalLayout[i].addView(mBitTextView[i], textLayoutParams);
    		horizontalLayout[i].addView(mBitValRadioButton[i], radioLayoutParams);
    		
    		LinearLayout.LayoutParams horizontalLayoutParams = new LinearLayout.LayoutParams(
   			     LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    		
    		mBitsLayout.addView(horizontalLayout[i], horizontalLayoutParams);	
		}
