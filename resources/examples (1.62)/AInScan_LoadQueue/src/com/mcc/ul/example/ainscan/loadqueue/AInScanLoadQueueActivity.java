package com.mcc.ul.example.ainscan.loadqueue;
...
import com.mcc.ul.AQueueElement;
import com.mcc.ul.AiDevice;
import com.mcc.ul.AiInfo;
import com.mcc.ul.AiQueueType;
...
import com.mcc.ul.example.ainscan.loadqueue.NetDiscoveryInfoDialog;
import com.mcc.ul.example.ainscan.loadqueue.NetDiscoveryInfoDialog.NoticeDialogListener;
import com.mcc.ul.example.ainscan.loadqueue.R;
...
/*
 * Library Call Demonstrated:    AiDevice.aLoadQueue()
   ...
   7. Call AiDevice.aLoadQueue to load channel/gain queue
   8. Call AiDevice.aInScan() to start the scan operation
   9. Call AiDevice.getStatus() to check the status of the background operation
   10. Call AiDevice.stopBackground() when scan is completed
...
public class AInScanLoadQueueActivity extends Activity {
	...
	CheckBox mChannelQueueCheckBox;
	CheckBox mGainQueueCheckBox;
    ...
    public class OnDaqDeviceSelectedListener implements OnItemSelectedListener {    
    	public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {      
        	...
        	if(devInfo.hasAiDev()) {
        		...
        		EnumSet<AiQueueType> aiQueueType = aiInfo.getQueueTypes();
        		
        		if(aiQueueType.contains(AiQueueType.CHANNEL))
        			mChannelQueueCheckBox.setEnabled(true);
        		else
        			mChannelQueueCheckBox.setEnabled(false);
        		
        		if(aiQueueType.contains(AiQueueType.GAIN))
        			mGainQueueCheckBox.setEnabled(true);
        		else
        			mGainQueueCheckBox.setEnabled(false);
    ...
    void startAInScan()
    {
    	...
    	int numChans = aiInfo.getNumChans(mode);
    	EnumSet<Range> ranges= aiInfo.getRanges(mode);
    	int samplesPerChan = Integer.parseInt(mSamplesPerChanEditText.getText().toString());
    	double rate = Double.parseDouble(mRateEditText.getText().toString());
    	int chanCount = 2;
    	int lowChan = 0; // This is ignored
    	int highChan = 0; // This is ignored
    	Range range = Range.BIP5VOLTS; //This is ignored
    	...
    	// create the queue
    	mAQueue = new AQueueElement[chanCount];
    	
    	for(int i = 0; i < chanCount; i++) {
    		mAQueue[i] = new AQueueElement();
    		mAQueue[i].mode = mode;
    	}
    	
    	if(mChannelQueueCheckBox.isChecked()) {
    		// Channel Queue
    		mAQueue[0].channel = 0;
    		mAQueue[1].channel = numChans - 1;
    	}
    	else {
    		mAQueue[0].channel = 0;
    		mAQueue[1].channel = 1;
    	...
    	if(!ranges.isEmpty()) {
	    	if(mGainQueueCheckBox.isChecked()) {
	    		// Gain Queue
	    		mAQueue[0].range = (Range) ranges.toArray()[0];
	    		
	    		if(ranges.size() > 1) 
	    			mAQueue[1].range = (Range) ranges.toArray()[1];
	    		else
	    			mAQueue[1].range = (Range) ranges.toArray()[0];
	    	}
	    	else {
	    		mAQueue[0].range = (Range) ranges.toArray()[0];
	    		mAQueue[1].range = (Range) ranges.toArray()[0];
    	...
    	try {
    		synchronized(this) {
    			if(mDaqDevice != null) {
					...
					Status scanStatus = mAiDevice.getStatus();
    ...
    void addScanDataTextViews(int chanCount) {
    	...
    	for(int i = 0; i < chanCount; i++) {
    		...
    		mScanChansTextView[i].setText("Channel " + mAQueue[i].channel + " (" + mAQueue[i].range +"):");
    ...
    private void initActivity() {
    	...
    	mChannelQueueCheckBox = (CheckBox) findViewById(R.id.checkbox_chanQueue);
    	mChannelQueueCheckBox.setEnabled(false);
    	...
    	mGainQueueCheckBox = (CheckBox) findViewById(R.id.checkbox_gainQueue);
    	mGainQueueCheckBox.setEnabled(false);
