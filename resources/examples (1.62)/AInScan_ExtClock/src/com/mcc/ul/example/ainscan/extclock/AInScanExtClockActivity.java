package com.mcc.ul.example.ainscan.extclock;
...
import com.mcc.ul.example.ainscan.extclock.NetDiscoveryInfoDialog;
import com.mcc.ul.example.ainscan.extclock.NetDiscoveryInfoDialog.NoticeDialogListener;
import com.mcc.ul.example.ainscan.extclock.R;
...
/*
 * Library Call Demonstrated:    AiDevice.aInScan()
  								 with AiScanOptions.EXTCLOCK scan option
   ...
   Demonstration:                Displays the analog input on user-specified channels.
   
   Other Library Calls:          AiDevice.getStatus()
                                 AiDevice.StopBackground()

   Special Requirements:         Selected device must have an A/D converter and
                                 support the AiScanOptions.EXTCLOCK option.
...
public class AInScanExtClockActivity extends Activity {
    ...
    public class OnDaqDeviceSelectedListener implements OnItemSelectedListener {    
    	public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {      
    		...
    		// Check if this DAQ Device has analog input device (subsystem)
        	if(devInfo.hasAiDev()) {
    ...
    void startAInScan()
    {
    	...
    	double rate = Double.parseDouble(mRateEditText.getText().toString());
    	EnumSet<AiScanOption> options = EnumSet.of(AiScanOption.EXTCLOCK);
