package com.mcc.ul.example.dbitout;
...
import com.mcc.ul.example.dbitout.NetDiscoveryInfoDialog.NoticeDialogListener;
import com.mcc.ul.example.dbitout.NetDiscoveryInfoDialog;
import com.mcc.ul.example.dbitout.R;
...
/*
 * Library Call Demonstrated:    DioDevice.dBitOut()

   Purpose:                      Sets the state of a single digital output bit.

   Demonstration:                Configures the selected bit or the selected port 
                                 for output (if necessary) then
                                 and writes a value to the bit.
                                 
   Other Library Calls:          DioDevice.dConfigPort()
   								 DioDevice.dConfigBit()
   ...
   7. Configure the bit as output ((if necessary)
   8. Call DioDevice.dBitOut() to set the state of the specified bit
...
public class DBitOutActivity extends Activity {
    ...
    public class OnPortSelectedListener implements OnItemSelectedListener {    
    	public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {      
    		PortInfo portInfo = mPortAdapter.getItem(mPortSpinner.getSelectedItemPosition());
    		
    		mBitAdapter.clear();
    		mBitAdapter.notifyDataSetInvalidated();
    		
    		for(int bit = 0; bit < portInfo.getNumBits(); bit++)
    			mBitAdapter.add(bit);
    ...
    public class OnBitSelectedListener implements OnItemSelectedListener {    
    	public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {
    		if(mDaqDevice.isConnected())
    			configureSelectedBit();
    	}    
    	public void onNothingSelected(AdapterView<?> parent){// Do nothing.
    	}
    }
    ...
    private void configureSelectedBit() {
    	PortInfo portInfo = mPortAdapter.getItem(mPortSpinner.getSelectedItemPosition());
    	int bitNum = mBitAdapter.getItem(mBitSpinner.getSelectedItemPosition());
    	
    	if(portInfo.getPortIOType() == DigitalPortIOType.BITIO || portInfo.getPortIOType() == DigitalPortIOType.IO) {
    		
    		try {
    			// If the specified port is bitwise configurable then configure the selected bit
    			if(portInfo.getPortIOType() == DigitalPortIOType.BITIO)
    				mDioDevice.dConfigBit(portInfo.getType(), bitNum, DigitalDirection.OUTPUT);
        		// Else configure the specified port if it is configurable
    			else if (portInfo.getPortIOType() == DigitalPortIOType.IO)
    ...
    private void writeBitState() {
    	
    	PortInfo portInfo = mPortAdapter.getItem(mPortSpinner.getSelectedItemPosition());
    	final int bitNum = mBitAdapter.getItem(mBitSpinner.getSelectedItemPosition());
    	boolean bitState = mBitState.isChecked();
    	String strBitState = bitState ?"High" : "Low";
    	
    	try {
    		// Set the specified bit
    		mDioDevice.dBitOut(portInfo.getType(), bitNum, bitState);
    		
    		updateStatus("Set bit " + bitNum + " state to " +  strBitState, false);
    ...
    private void initActivity() {
    	...
    	mBitSpinner = (Spinner) findViewById(R.id.spinner_bit);
    	mBitAdapter =  new ArrayAdapter <Integer> (this, android.R.layout.simple_spinner_item );
    	mBitAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    	mBitSpinner.setAdapter(mBitAdapter);
    	mBitSpinner.setOnItemSelectedListener(new OnBitSelectedListener());
    	mBitSpinner.setEnabled(false);
    	...
    	mBitState = (ToggleButton) findViewById(R.id.toggleButton_bitState);
